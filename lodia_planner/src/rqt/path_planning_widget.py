...
from lodia_planner.globalplanner import transform
from lodia_planner.rqt.map_widget import MapWidget
import tf
...
class PathPlanningWidget(QtWidgets.QWidget):
    ...
    def __init__(self, context, setup):
        ...
        ui_file = os.path.join(self.path_to_root, 'ui', 'PathPlanningWidget.ui')
        loadUi(ui_file, self)
        ...
        #self.setup = setup_file.Setup(self.path_to_root, 0.5, 0.5, 0)
        self.setup = setup
        self.wp_pixel = None
        self.wp_global = None
        self.wp_sim = None
        self.path_pixel = None
        self.REALLIFE = rospy.get_param('/lodia_plugin/planner/reallife', True)
        self.BASE_FRAME = rospy.get_param('/lodia_plugin/common/base_frame', 'base')
        self.ODOM_FRAME = rospy.get_param('/lodia_plugin/common/odom_frame', 'odom')
        ...
        # Try inhouse
        #(trans,_) = self.tf_listener.lookupTransform('/map', '/base', rospy.Time(0))
        # self.sub_current_pos = rospy.Subscriber('/rtk_gps_driver/position_receiver_0/ros/navsatfix', \
        #                                             NavSatFix, self.update_tracked_path_reallife)
        # self.start = np.array([trans[0], trans[1]])
        # self.goal = np.array([self.start[0], self.start[1]+10])

        # Define the start and goal positions
        self.sub_current_pos = rospy.Subscriber('/rtk_gps_driver/position_receiver_0/ros/navsatfix', \
                                                NavSatFix, self.update_current_pose)
        self.current_pose = (0, 0)
        rospy.sleep(1)

        # TODO: changed for Allmend
        self.start = (0, 0)
        self.start = self.get_robot_position()
        self.goal = np.array([self.start[0], self.start[1]+10])
        # start = (8.51816, 47.35655); goal = (8.51854, 47.3553)
        # goal = (8.5192, 47.3556)
        # [self.goal] = transform.from_globe_to_sim([(8.5192, 47.3556)], setup)
        # start = (80, 175)
        #self.goal = (110, 40)
        # self.start = (752.5, 105.9)
        # self.goal = (77.2, 645.0)
        #Moon:
        # self.start = (746, 83)
        # self.goal = (134, 160)
        #Allmend
    ...
    def update_current_pose(self, msg):
        self.current_pose = (msg.longitude, msg.latitude)


    def load_and_plot_coordinates(self):
        self.mapwidget.plot_point_on_canvas(self.start, 'old_start')
        self.mapwidget.plot_point_on_canvas(self.goal, 'old_goal')
    ...
    def calc_and_plot_path(self):
        ...
        # Plan path
        self.start = self.get_robot_position()
        rospy.loginfo("Calculation of global path started.")
        self.calc_path(self.start, self.goal)
        rospy.loginfo("Global path successfully calculated.")
        self.plot_layer_with_start_goal_and_path()
    ...
    def click_on_map_sets_new_goal(self, event):
        ...
        try:
            ...
            if self.setup.maps.maps_array[index_x, index_y, 4] != 1:
                ...
                rospy.logerr("A point in a banned area was chosen. Please choose a new goal by klicking with the right mouse button on the map.")

        except (ValueError, TypeError):
            rospy.logerr("Goal outside map chosen. Choose a next goal while klicking with the right mouse button on the map.")
    ...
    def get_robot_position(self):
        ...
        Returns the current robot position
            Parameters:
                globe (Boolean): Flag if position should be returned in globe coordinates or sim coordinates
        '''
        # Overwrite if in real life
        if self.REALLIFE:
            if self.current_pose == (0, 0):
                current_pos = self.start
            else:
                current_pos = self.current_pose
                [current_pos] = transform.from_globe_to_sim([current_pos], self.setup)
        elif self.PLOT_GLOBE:
            (trans,_) = self.tf_listener.lookupTransform(self.ODOM_FRAME, self.BASE_FRAME, rospy.Time(0))
            current_pos = np.array(transform.from_sim_to_globe([trans[0], trans[1]], self.setup))
        else:
            (trans,_) = self.tf_listener.lookupTransform(self.ODOM_FRAME, self.BASE_FRAME, rospy.Time(0))
            current_pos = np.array([trans[0], trans[1]])
    ...
    def calc_path(self, start, goal):
        ...
        self.path_pixel, self.stats = astar_file.astar(self.setup.map_size_in_pixel, start_pixel, goal_pixel, \
                                           self.setup.h_func, self.setup.g_func, allow_diagonal=True)
        if self.path_pixel.any() == -1:
            rospy.logerr("No valid path found! Please check input parameters.")
        ...
        np.savetxt(self.path_to_root+'/data/waypoints.dat', wp_all, \
        ...
        np.savetxt(self.path_to_root+'/data/stats.dat', stats_with_wp, \
    ...
    def add_path0(self):
        ...
        self.add_pathx(0)
        rospy.loginfo("Saved as path 1.")

    def add_path1(self):
        '''Plots path 1 in the mapwidget cmp'''
        self.add_pathx(1)
        rospy.loginfo("Saved as path 2.")

    def add_path2(self):
        '''Plots path 2 in the mapwidget cmp'''
        self.add_pathx(2)
        rospy.loginfo("Saved as path 3.")

    def add_path3(self):
        '''Plots path 3 in the mapwidget cmp'''
        self.add_pathx(3)
        rospy.loginfo("Saved as path 4.")
    ...
    def execute_pathx(self, x):
        ...
        rospy.loginfo('Path '+str(x+1)+' chosen.')
    ...
    def shutdown_plugin(self):
        ...
        rospy.loginfo("Shutdown Path Planning Widget.")
